...
def adjacency_matrix_to_pydot(adj_matrix, feature_names):
    # 创建有向图
    graph = pydot.Dot(graph_type='digraph', dpi=200)

    # 添加节点
    nodes = []
    for idx, name in enumerate(feature_names):
        node = pydot.Node(idx, label=name)
        graph.add_node(node)
        nodes.append(node)
    ...
    for i in range(num_nodes):
        for j in range(num_nodes):
            if adj_matrix[i, j] != 0:
                edge = pydot.Edge(nodes[i], nodes[j], arrowhead='none', arrowtail='none', dir='both')
                graph.add_edge(edge)

    # 设置图属性
    graph.set('dpi', '200')
    graph.set('fontsize', '18')
...
class DAGMA_linear:
    ...
    def fit(self, X, lambda1, w_threshold=0.3, T=5,
        ...
        if self.loss_type == 'l2':
            ...
            self.X -= X.mean(axis=0, keepdims=True).astype(int)
        ...
        with tqdm(total=(T - 1) * warm_iter + max_iter) as pbar:
            for i in range(int(T)):
                ...
                while success is False:
                    W_temp, success = self.minimize(self.W_est.copy(), mu, inner_iters, s[i], lr=lr_adam, beta_1=beta_1,
